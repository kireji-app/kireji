 ...
 log => {
  ...
  try {
   respond: {
    ...
    status = !isSupportedFile && _.openTask.prototype.host === "error.abstract.parts" ? _.openTask.status : 200
    head = {
   ...
   // Let the offline server field the request.
...
globalThis.onmessage = ({ data: { code, payload }, source }) => {
 switch (code) {
  ...
  default:
   logError('Unsupported Offline Server Message: ' + code)
 }
}
