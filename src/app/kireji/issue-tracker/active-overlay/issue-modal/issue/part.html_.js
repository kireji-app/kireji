...
return (
 ...
 `<p class=issue-description data-priority="${thisKirejiIssue.priority}" data-status="${thisKirejiIssue.status}">` + thisKirejiIssue.wrap(
  ...
  `<b>Affected Parts:</b>\n<span class="issue-affects">${thisKirejiIssue.affects.map(affected => affected === _ ? _.key : affected.host).join(", ") || "None"}</span>\n\n` +
